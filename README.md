...
`POST /api/v1/clips` accepts a small, fully-encoded MP4 file as base64 (bare base64 or a `data:video/mp4;base64,` URL) in the `mp4` field, plus optional metadata: `filename` (the original name of the video file), `author` (free-text, self-reported submitter), `frame_start`/`frame_end` (the frame subset this clip covers in the original video, provided together), and `timestamp`. There is no source `video_url` in this design.

The server verifies the bytes with `ffprobe` (they must contain a decodable video stream), then uploads TWO files per submission: the clip itself and a JSON provenance sidecar carrying all of the metadata above plus the server-derived fields (submission id, JWT-derived submitter when present, size, codec-free record of what was sent). The uploaded clip is named after the original video with the frame subset tagged on, e.g. `mice_frames-100-180_03ff05e9.mp4` (a short submission-id suffix keeps names unique), and the sidecar shares that name (`mice_frames-100-180_03ff05e9.json`). Without a `filename` the name falls back to the hour tag plus submission id. Both files are written into dandiset `000474` and pushed with a synchronous `dandi upload --allow-any-path --validation skip` inside the request. By default the CLI only discovers recognized asset types (NWB, zarr, video), silently skipping the `.json` sidecar; `--allow-any-path` uploads every file but is a development option that only exists when `DANDI_DEVEL=1` is set in the subprocess environment (the server sets it on the upload call). Validation is skipped because neither file is an NWB asset. There is no buffer and no cron involvement for clips.
