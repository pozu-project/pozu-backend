...
`POST /api/v1/clips` accepts a small, fully-encoded MP4 file as base64 (bare base64 or a `data:video/mp4;base64,` URL) in the `mp4` field, plus an optional `filename` (the original name of the video file) and `timestamp`. There is no source `video_url` in this design. The server verifies the bytes with `ffprobe` (they must contain a decodable video stream), then uploads TWO files per submission: the clip itself and a basic JSON provenance sidecar (original filename, who is submitting, size, timestamp). Both are written into dandiset `000474` and pushed with a synchronous `dandi upload --validation skip` inside the request; validation is skipped because neither file is an NWB asset. There is no buffer and no cron involvement for clips.
