...
def decode_clip_mp4(mp4_value, /) -> bytes:
    """Validate and decode a pre-encoded MP4 posted as base64.
    ...
    try:
        ...
        raise BadRequest("'mp4' is not valid base64")

    if len(blob) > MAX_CLIP_MP4_BYTES:
        raise BadRequest(f"'mp4' exceeds the {MAX_CLIP_MP4_BYTES} byte limit")
    if len(blob) < 12 or blob[4:8] != MP4_FTYP_TAG:
        raise BadRequest("'mp4' does not look like an MP4 file (missing ftyp signature)")
    return blob
...
def upload_clip_to_dandi(clip_paths, /) -> None:
    ...
    # By default `dandi upload` only discovers recognized asset types
    # (.nwb/.zarr/.ngff and videos), silently skipping the JSON sidecar.
    # --allow-any-path uploads every file, but it is a development option that
    # only exists when DANDI_DEVEL=1 is set in the environment. Validation is
    # skipped because neither file is an NWB asset.
    env["DANDI_DEVEL"] = "1"
